...
func pairLines(lines []DiffLine) []linePair {
	...
	for i < len(lines) {
		...
		case LineRemoved:
			...
			maxCount := max(removedCount, addedCount)

			for j := 0; j < maxCount; j++ {
				var leftLine, rightLine *DiffLine
				if j < removedCount {
					leftLine = &lines[removedStart+j]
				}
				if j < addedCount {
					rightLine = &lines[addedStart+j]
				}
				pairs = append(pairs, linePair{left: leftLine, right: rightLine})
			}

		case LineAdded:
			// Standalone added line (not part of a removed->added block)
...
func getColor(adaptiveColor compat.AdaptiveColor) *string {
	// Create a temporary style to extract the color
...
func createStyles(t theme.Theme) (removedLineStyle, addedLineStyle, contextLineStyle, lineNumberStyle lipgloss.Style) {
	...
	lineNumberStyle = lipgloss.NewStyle().Foreground(t.TextMuted()).Background(t.DiffLineNumber())
	return
}
...
func renderLinePrefix(dl DiffLine, lineNum string, marker string, lineNumberStyle lipgloss.Style, t theme.Theme) string {
	...
	switch dl.Kind {
	case LineRemoved:
		styledMarker = lipgloss.NewStyle().Foreground(t.DiffRemoved()).Background(t.DiffRemovedBg()).Render(marker)
	case LineAdded:
		styledMarker = lipgloss.NewStyle().Foreground(t.DiffAdded()).Background(t.DiffAddedBg()).Render(marker)
	case LineContext:
		styledMarker = lipgloss.NewStyle().Foreground(t.TextMuted()).Background(t.DiffContextBg()).Render(marker)
	default:
		styledMarker = marker
	}
...
func renderLineContent(fileName string, dl DiffLine, bgStyle lipgloss.Style, highlightColor compat.AdaptiveColor, width int) string {

...
Do NOT attempt to run the `dev serve` command. The `dev serve` server is automatically reloaded on change. Just CURL the API if you want to test a change.

## Code Style Guidelines

IMPORTANT:

- Do not use pointers unless required for mutation or other purposes. Returning an empty struct on errors rather than nil is preferable.
...
- Wrap errors with context using `fmt.Errorf("actioning: %w", err)`
